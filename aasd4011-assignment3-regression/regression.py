...
def fit_regression_model(X, y):
    ...
    num_epochs = 10001 # Pick a better number of epochs
    ...
    for epoch in range(1, num_epochs):
        ...
        if previos_loss-loss.item() < 0.01: # Change this condition to stop the training when the loss is not changing much.
            break
        previos_loss = loss.item()
        if epoch%1000 == 0:
            print(f"Epoch {epoch}: Loss value is {previos_loss}")
